defmodule GameOfTag.GameState do
  ...
  defp reset_player_position(player),
    do: Map.merge(player, %{x: @board_size / 2, y: @board_size / 2})
  ...
  def move_player(player_id, direction) do
    delta =
      case direction do
        "right" -> %{x: 1, y: 0}
        "left" -> %{x: -1, y: 0}
        # canvas coordinates start from top
        "up" -> %{x: 0, y: -1}
        "down" -> %{x: 0, y: 1}
      end
  ...
  defp detect_collision(player) do
    players()
    |> Map.values()
    |> Enum.find(fn p -> players_in_same_position(p, player) end)
  end
